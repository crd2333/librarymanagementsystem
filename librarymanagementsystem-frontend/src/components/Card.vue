            ...
            <div style="position:relative;" class="cardBox" v-for="card in cards" v-show="card.name.includes(toSearch)" :key="card.id">
                <div>
                    ...
                    <div style="position:absolute; margin-bottom: 10px; bottom:0; margin-left: 45px">
                        <el-button type="primary" :icon="Edit" circle @click="this.toModifyInfo.id = card.id, this.toModifyInfo.name = card.name, this.toModifyInfo.department = card.department, this.toModifyInfo.type = card.type, this.modifyCardVisible = true"  />
        ...
        <el-dialog v-model="newCardVisible" title="新建借书证" width="30%" align-center>
            ...
            <template #footer>
                <span>
                    ...
                    <el-button type="primary" @click="ConfirmNewCard" :disabled="newCardInfo.name.length === 0 || newCardInfo.department.length === 0">确定</el-button>
                </span>
            </template>
        </el-dialog>
    ...
    methods: {
        ConfirmNewCard() {
            ...
            axios.post("/card",
                ...
                .then(response => {
                    if (response.data == "Register card successfully!")
                        ElMessage.success("借书证创建成功") // 显示消息提醒
                    else
                        ElMessage.error("借书证创建失败")
        ...
        ConfirmModifyCard() {
            ...
            axios.patch("/card",
                ...
                .then(response => {
                    if (response.data == "Modify card successfully!")
        ...
        ConfirmRemoveCard() {
            ...
            //         this.QueryCards() // 重新查询借书证以刷新页面
            //     })
            axios.delete("/card?id=" + this.toRemove)
                .then(response => {
                    if (response.data == "Remove card successfully!")
                        ElMessage.success("借书证删除成功") // 显示消息提醒
                    else
